...
export async function POST(req: NextRequest){
    ...
    try{
        ...
        return NextResponse.json({success: true, message: "message added successfully", result: result},{status: 202});
    }catch (e){
...
export async function GET(req: NextRequest){
    ...
    try{
        ...
        const result = await directMessage.findOne(
            { _id: new ObjectId(GroupID)},{messages:1});
        ...
        return NextResponse.json({success: true, message: "here is the list of messages", sortedMessages: sortedMessages},{status: 202});
    }catch (e){
        return NextResponse.json({success: false, message: e.message},{status: 500});
    }
}